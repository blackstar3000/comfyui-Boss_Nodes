"""Fetch character preview images from nax.moe for the Character Selector Editor.

Usage:
    python fetch_character_previews.py          # Fetch all pages
    python fetch_character_previews.py --fast   # First 5 pages only
    python fetch_character_previews.py --resume # Skip already-fetched pages

Saves to: py/character_previews.json
"""
...
GALLERY_URL = "https://nax.moe/?gallery=danbooru-character-tags-v4.5&sort=date&page={page}"
OUTPUT_FILE = Path(__file__).parent / "character_previews.json"
CACHE_FILE = Path(__file__).parent / ".character_previews_cache.json"
REQUEST_DELAY = 1.0
SAVE_INTERVAL = 5

# Regex patterns for parsing nax.moe HTML
# Matches copy('danbooru-character-tags-v4.5', 'CHARACTER_TAG') calls — the primary source of character names
COPY_FN_RE = re.compile(r"copy\('danbooru-character-tags-v4\.5',\s*'([^']+)'\)")
# Matches CDN image URLs in img src
CDN_IMG_RE = re.compile(r'src="(https://cdn\.zele\.st/data/NAX/Images/danbooru-character-tags-v4\.5/[^"?]+\.jpg)"', re.IGNORECASE)
...
def parse_page(html):
    """Extract character tags and image URLs from HTML page.

    The nax.moe gallery page has a consistent structure where each item contains:
    - onclick="copy('danbooru-character-tags-v4.5', 'CHARACTER_TAG')" for the tag name
    - <img src="CDN_URL" ...> for the preview image
    These appear in matching order within the HTML.
    """
    results = {}

    # Primary: extract character tags from onclick copy() calls
    character_tags = COPY_FN_RE.findall(html)

    # Extract CDN image URLs
    cdn_images = CDN_IMG_RE.findall(html)

    if not character_tags or not cdn_images:
        return results
...
def load_existing():
    """Load existing character previews from file."""
...
def save_previews(previews):
    """Save character previews to file."""
    with OUTPUT_FILE.open("w", encoding="utf-8") as f:
        json.dump(previews, f, ensure_ascii=False, indent=2)


def main():
    parser = argparse.ArgumentParser(description="Fetch character previews from nax.moe")
    ...
    print(f"Fetching character previews from nax.moe ({mode} mode)...")
    ...
    while consecutive_empty < 3:
        ...
        if not results:
            ...
            print(f"  Page {page}: found {len(results)} characters (total: {len(previews)})")
    ...
    print(f"\nDone! Saved {len(previews)} character previews to {OUTPUT_FILE}")
