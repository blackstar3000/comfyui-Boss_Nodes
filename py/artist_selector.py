...
def _get_artist_categories_file(force_refresh: bool = False):
    """Load and cache artist_categories.json. Returns (categories_list, artist_categories_dict)."""
...
def _get_database(force_refresh: bool = False):
    """Load and cache the artist database. Returns 7-tuple:
    (library, favorites, history, previews, categories_list, artist_categories, post_counts).
    """
    global _db_cache
    if force_refresh or _db_cache is None:
        ...
        categories_list, cat_map = _get_artist_categories_file(force_refresh=force_refresh)
        ...
        _db_cache = (library, favorites, history, previews, categories_list, cat_map, post_counts)
    return _db_cache
...
def _set_favorites(favorites: list):
    ...
    global _db_cache
    library, _, history, previews, categories_list, cat_map, post_counts = _get_database(force_refresh=False)
    _db_cache = (library, list(favorites), history, previews, categories_list, cat_map, post_counts)
...
class BossArtistSelector:
    ...
    def select(self, selection, max_artists, randomize, favorites_only,
        ...
        try:
            library, favorites, history, previews, categories_list, cat_map, post_counts = _get_database(force_refresh=force_refresh)
        except Exception:
            return ("", "", 0)
        ...
        if save_json(HISTORY_FILE, history):
            ...
            if _db_cache is not None:
                lib, fav, _, previews, categories_list, cat_map, post_counts = _db_cache
                _db_cache = (lib, fav, history, previews, categories_list, cat_map, post_counts)
...
def register_api_routes():
    ...
    async def get_artists_data(request):
        try:
            library, favorites, history, previews, categories_list, cat_map, post_counts = _get_database(force_refresh=False)
